...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/admin/users', async (req: any, res) => {
    try {
      ...
      const updated = await (await import('./githubService')).githubService.createUserWithCredentials({ email, password, role, firstName, lastName, permissions, isActive });
  ...
  app.post('/api/admin/users/:userId/password', async (req: any, res) => {
    try {
      ...
      const updated = await (await import('./githubService')).githubService.setUserPassword(userId, password);
  ...
  app.post("/api/custom-messages/reset", isAuthenticated, async (req, res) => {
    try {
      const messages = await storage.resetCustomMessages();
      res.json(messages);
    } catch (error) {
